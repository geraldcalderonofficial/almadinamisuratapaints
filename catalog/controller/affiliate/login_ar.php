<?php 
class ControllerAffiliateLoginAr extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && isset($this->request->post['email']) && isset($this->request->post['password']) && $this->validate()) {
      	...
      	$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('affiliate/login_ar', '', 'SSL'),      	
        	'separator' => $this->language->get('text_separator')
      	);
		...
		$this->data['action'] = $this->url->link('affiliate/login_ar', '', 'SSL');
		$this->data['register'] = $this->url->link('affiliate/register_ar', '', 'SSL');
		$this->data['forgotten'] = $this->url->link('affiliate/forgotten_ar', '', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/affiliate/login_ar.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/affiliate/login_ar.tpl';
		} else {
			$this->template = 'default/template/affiliate/login_ar.tpl';
		}
