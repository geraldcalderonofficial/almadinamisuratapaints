<?php 
class ControllerAccountRecurringAr extends Controller {
    public function index() {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('account/recurring_ar', $url, 'SSL'),
            'separator' => $this->language->get('text_separator')
        );
        ...
        if($results){
            foreach ($results as $result) {
                $this->data['profiles'][] = array(
                    ...
                    'href'                  => $this->url->link('account/recurring_ar/info','recurring_id='.$result['order_recurring_id'],'SSL'),
                );
            }
        }
        ...
        $pagination->url = $this->url->link('account/recurring_ar', 'page={page}', 'SSL');

        $this->data['pagination'] = $pagination->render();

        $this->data['continue'] = $this->url->link('account/account_ar', '', 'SSL');

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/recurring_list_ar.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/account/recurring_list_ar.tpl';
        } else {
            $this->template = 'default/template/account/recurring_list_ar.tpl';
        }
    ...
    public function info() {
        ...
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('account/recurring_ar/info', 'recurring_id=' . $recurring_id, 'SSL');
        ...
        $profile['product_link'] = $this->url->link('product/product_ar', 'product_id='.$profile['product_id'], 'SSL');
        $profile['order_link'] = $this->url->link('account/order_ar/info', 'order_id='.$profile['order_id'], 'SSL');
        ...
        if ($profile) {
            ...
            $this->data['breadcrumbs'][] = array(
                ...
                'href'      => $this->url->link('account/recurring_ar/info', 'recurring_id=' . $this->request->get['recurring_id'] . $url, 'SSL'),
                'separator' => $this->language->get('text_separator')
            );
            ...
            $this->data['continue'] = $this->url->link('account/recurring_ar', '', 'SSL');
            $this->data['profile'] = $profile;

            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/recurring_info_ar.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/account/recurring_info_ar.tpl';
            } else {
                $this->template = 'default/template/account/recurring_info_ar.tpl';
            }
            ...
            $this->redirect($this->url->link('account/recurring_ar', '', 'SSL'));
        }
    }
}
?>
